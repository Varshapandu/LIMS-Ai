...
def create_invoice(payload: CreateInvoiceRequest, db: Session = Depends(get_db)) -> CreateInvoiceResponse:
    try:
...
def get_invoice_summary(invoice_number: str, db: Session = Depends(get_db)) -> InvoiceSummaryResponse:
    try:
...
def record_payment(payload: PaymentRequest, db: Session = Depends(get_db)) -> PaymentResponse:
    try:
